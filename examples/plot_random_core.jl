...
function add_plot_info(
    plt::AbstractPlot; extra::AbstractString = string(), counter::Integer = 0
)
...
function graphing(
    ...
    n::Integer,
    d::Integer,
    data::Union{AbstractArray, DataFrame},
    stop_at::Integer;
    m::Integer = 10_000,
)
    save_path = joinpath(
        ...
        "random_number_in_code,q=$(q),n=$(n),d=$(d),i=$(stop_at),m=$(m).pdf",
    )
    ...
    bin_adjustment = ifelse(
        🐖_obtained, 0.5, minimum(abs.(extrema(vcat(data, x_max)) .- 🐖)) + 0.5
    ) # ensure the bins encompass the greedy
    ...
    if isempty(data)
        add_plot_info(plt; extra = "🐖 = $(🐖)", counter = annotation_counter)
    ...
    if !🍖_annotated
        annotate!(
            plt,
            maximum(filter(!isnan, plt.series_list[1].plotattributes[:x])) -
            x_annotation_factor,
            maximum(filter(!isnan, plt.series_list[1].plotattributes[:y])) + 0,
            "🍖 = $(🍖)",
        )
    ...
    if !🐺_annotated
        annotate!(
            ...
            maximum(filter(!isnan, plt.series_list[1].plotattributes[:x])) -
            x_annotation_factor,
            maximum(filter(!isnan, plt.series_list[1].plotattributes[:y])) -
            y_annotation_factor,
            "🐺 = $(🐺)",
        )
        # add_plot_info(plt, extra="🐺 = $(🐺)", counter=annotation_counter); annotation_counter += 1
