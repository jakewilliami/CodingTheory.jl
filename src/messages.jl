...
"""
function push_if_allowed!(
    C::AbstractVector{T}, C′::AbstractVector{T}, w::T, d::Int
) where {T <: AbstractWord}
    isempty(C) && (push!(C′, w); return true)
...
"""
function replace_if_allowed!(
    C::AbstractVector{T}, d::Int, ws::Pair
) where {T <: AbstractWord}
    w, w′ = ws
...
function replace_if_allowed!(
    C::AbstractVector{T}, d::Int, w::T, w′::T
) where {T <: AbstractWord}
    return replace_if_allowed!(C, d, Pair(w, w′))
end

_mutate_codeword(w::Word{N, T}, i::Int, a::T) where {T, N} = setindex!(w, a, i)

"""
```julia
mutate_codeword(w::NonStaticAbstractWord{N, T}, n::Int, i::Int, a::T) where {T, N} ->
    MVector{N, T}
...
function mutate_codeword(w::Word{N, T}, n::Int, i::Int, a::T) where {T, N}
    return _mutate_codeword(w, i, a)
end
...
get_all_words(Σ::Alphabet{N}, n::Int) where {N} = get_all_words(Σ, length(Set(Σ)), n) # if alphabet is given, then q is the length of that alphabet
get_all_words(Σ::AbstractArray, n::Int) = get_all_words(Alphabet(Σ), length(Set(Σ)), n) # if alphabet is given, then q is the length of that alphabet
get_all_words(q::Int, n::Int) = get_all_words(genalphabet(q), q, n) # generate symbols if no alphabet is given
...
"""
```julia
get_codewords(𝒰::UniverseParameters, d::Int; m::Int = 10, m_random::Int = 1000) ->
    Codewords{M}
...
get_codewords(q::Int, n::Int, d::Int; m::Int = 10, m_random::Int = 1000) -> Codewords{M}
get_codewords(
    Σ::AbstractArray, q::Int, n::Int, d::Int; m::Int = 10, m_random::Int = 1000
) -> Codewords{M}
...
function get_codewords(
    Σ::Alphabet{N}, q::Int, n::Int, d::Int; m::Int = 10, m_random::Int = 1000
) where {N}
    return get_codewords(UniverseParameters(Σ, q, n), d; m = m, m_random = m_random)
end
function get_codewords(
    Σ::Alphabet{N}, n::Int, d::Int; m::Int = 10, m_random::Int = 1000
) where {N}
    return get_codewords(UniverseParameters(Σ, n), d; m = m, m_random = m_random)
end
function get_codewords(q::Int, n::Int, d::Int; m::Int = 10, m_random::Int = 1000)
    return get_codewords(UniverseParameters(q, n), d; m = m, m_random = m_random)
end
function get_codewords(
    Σ::AbstractArray, q::Int, n::Int, d::Int; m::Int = 10, m_random::Int = 1000
)
    return get_codewords(Alphabet(Σ), q, n, d; m = m, m_random = m_random)
end
function get_codewords(Σ::AbstractArray, n::Int, d::Int; m::Int = 10, m_random::Int = 1000)
    return get_codewords(UniverseParameters(Σ, n), d; m = m, m_random = m_random)
end
function get_codewords(
    Σ::AbstractArray,
    ...
    return get_codewords(Alphabet(Σ), q, n, d, 𝒰; m = m, m_random = m_random)
end
