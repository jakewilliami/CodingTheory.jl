...
function Ω(n)
    ...
    return sum(e for (_, e) in _factors(n))
end
...
function __newton_nroot(y::Real, k::Real, b::Real)
    ...
    z = ifelse(
        b′ ≤ ceil(Int, lg(8 * k)), __nroot(__nroot(y, k, b′)), __newton_nroot(y, k, b′)
    ) # if else, then b′ ≥ ceil(Int, lg(8*k)) + 1
    r₂ = trunc(z, B) * (k + 1)
