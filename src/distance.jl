...
function __hamming_space(
    relation::Function, Σⁿ::AbstractArray{T}, w::AbstractArray, e::Int
...
```
"""
function t_error_detecting(
    C::AbstractArray{T}, t::Int
) where {T <: Union{AbstractArray{Int}, AbstractWord}}
...
```
"""
function find_error_detection_max(
    C::AbstractArray{T}, modulo::Int
) where {T <: Union{AbstractArray{Int}, AbstractWord}}
...
"""
function find_error_correction_max(
    C::AbstractArray{T}, modulo::Int
) where {T <: Union{AbstractArray{Int}, AbstractWord}}
    for t in (modulo - 1):-1:0
